...
/**
 * The Class ImageLoader is the Image utility class that is used to download the
 * image from URL in many possible ways.
 */
public class ImageLoader extends ImageUtils
{
	...
	/**
	 * The Class QueueItem is used to represent an item in image listing queue,
	 */
	...
	/**
	 * The listener interface for receiving imageLoaded events. The class that
	 * is interested in processing a imageLoaded event implements this
	 * interface, and the object created with that class is registered with a
	 * component using the component's
	 * <code>addImageLoadedListener<code> method. When
	...
	private Bitmap readBitmapFromNetwork(String urlStr)
	{
		...
		try
		{
			...
			if (w == 0 && h == 0)
			{
				...
				bmp = getCompressedBm(buf, w, h, scale);
